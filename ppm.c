...
Image *readPPMHeader(FILE *f_source, Image *buffer){
    ...
    buffer->data = (u_char *)malloc(buffer->width*buffer->height*3);// G, R, B three colors
    ...
    if(format==6){
        ...
        for(i=0;i<buffer->height;i++){
            for(j=0;j<buffer->width;j++){
                ...
                fprintf(f_des, "%d\n",buffer->data[i*buffer->width*4+4*j]);
                fprintf(f_des, "%d\n",buffer->data[i*buffer->width*4+4*j+1]);
                fprintf(f_des, "%d\n",buffer->data[i*buffer->width*4+4*j+2]);
                //Skip the A
            }
        ...
        if (strcmp(buffer->tupltype, "RGB_ALPHA") == 0) {
            num2 = fwrite((void *) buffer->data, 1, (size_t) size, f_des);
        }else if (strcmp(buffer->tupltype, "RGB")==0){
            for(int i=1; i<size+1;i++){
                char ch=buffer->data[i-1];
                if (i%4 !=0) {
